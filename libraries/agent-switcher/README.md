...
```python
from agent_switcher import Agent
...
```python
from agent_switcher import strip_think_blocks, serialize_for_json, extract_tool_calls
```
...
Uses [uv](https://docs.astral.sh/uv/) for the venv and running tools. From `libraries/agent-switcher`:
