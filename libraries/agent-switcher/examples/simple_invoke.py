...
try:
    ...
    # Load .env from agent-switcher root (parent of examples/) so it works when run via scripts/simple_invoke.sh
...
# Add src so agent_switcher is importable when run from repo
